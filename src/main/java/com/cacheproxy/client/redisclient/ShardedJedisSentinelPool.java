...
public final class ShardedJedisSentinelPool extends Pool<ShardedJedis> {
    ...
    private List<JedisShardInfo> makeShardInfoList(List<HostAndPort> masters) {
        ...
        for (HostAndPort master : masters) {
            JedisShardInfo jedisShardInfo =
                    new JedisShardInfo(master.getHost(), master.getPort(), timeout);
            ...
            while (!fetched && sentinelRetry < MAX_RETRY_SENTINEL) {
                for (String sentinel : sentinels) {
                    ...
                    try {
                        ...
                        logger.warn("Cannot connect to sentinel running @ " + hap
                                + ". Trying next one.");
                    }
                }
        ...
        if (masters.size() != 0 && masters.size() == shardMasters.size()) {
            ...
            for (String sentinel : sentinels) {
                ...
                MasterListener masterListener =
                        new MasterListener(masters, hap.getHost(), hap.getPort());
    ...
    private class MasterListener extends Thread {
        ...
        public void run() {
            ...
            while (running.get()) {
                ...
                try {
                            ...
                            if (switchMasterMsg.length > 3) {
                                ...
                                if (index >= 0) {
                                    ...
                                    logger.info("Ignoring message on +switch-master for master name "
                                            + switchMasterMsg[0] + ", our monitor master name are ["
                                            + sb + "]");
                                }

                            } else {
                                logger.info("Invalid message received on Sentinel " + host + ":"
                    ...
                    if (running.get()) {
                        logger.info("Lost connection to Sentinel at " + host + ":" + port
        ...
        public void shutdown() {
            try {
                ...
                logger.info("Caught exception while shutting down: " + e.getMessage());
            }
        }
    }
